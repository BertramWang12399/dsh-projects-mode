// dsh-projects-mode v0.3 — Browser half (ModuleLoader bundle).
...
// v0.3 adds token meters for instructions/memory, the auto-briefing toggle,
// and a per-project context cost estimate (the token-saving story).
        ...
        function buildSkeleton() {
          ...
          newSessBtn.addEventListener('click', function () {
            ...
            pollSnapshot(Date.now() + 5000)
              ...
              .then(function (sid) {
                if (!sid) return null
                return api('assign', { sessionId: sid, projectId: pid }).then(function () {
                  return sid
                })
              })
              .then(function (sid) {
          ...
          foot.appendChild(h('span', null, 'v0.3.2 · 项目简报=新会话自动接续上下文，比拖长老会话省 token'))
