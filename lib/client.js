// dsh-projects-mode — Browser half (ModuleLoader bundle).
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Required services: sessions for one-click open. */
    ...
    function apply(ctx) {
        ...
        function buildSkeleton() {
          ...
          newSessBtn.addEventListener('click', function () {
            ...
            if (workspaces && typeof workspaces.startSession === 'function') {
              createPromise = Promise.resolve().then(function () {
                const before = {}
                model.sessions.forEach(function (s) {
                  before[s.id] = true
                })
                workspaces.startSession(undefined)
                return pollForNew(before, Date.now() + 8000)
              })
            } else if (svc && typeof svc.create === 'function') {
              createPromise = svc.create({})
            } else {
              createPromise = Promise.reject(new Error('无法创建会话：客户端服务不可用'))
            }
            createPromise
              .then(function (sid) {
                if (!sid) throw new Error('未能捕获新会话')
                return api('assign', { sessionId: sid, projectId: model.selected }).then(function () {
                  return sid
                })
              })
              .then(function (sid) {
          ...
          // place on the next step after each save).
          ...
          addBtn.addEventListener('click', function () {
            ...
            api('assign', { sessionId: pickSel.value, projectId: model.selected })
              .then(function () {
                return refresh()
              })
              .catch(function (e) {
        ...
        function renderSessions() {
          ...
          mine.forEach(function (s) {
            ...
            rm.addEventListener('click', function (e) {
              e.stopPropagation()
              api('assign', { sessionId: s.id, projectId: null })
                .then(function () {
                  return refresh()
                })
                .catch(function (err) {
      ...
      function mountBadge() {
        ...
        function buildMenu() {
          ...
          api('state')
            .then(function (st) {
              ...
              const asg = st.assignments || {}
              const curPid = asg[currentSessionId] || null
        ...
        function assignTo(pid) {
          hideMenu()
          api('assign', { sessionId: currentSessionId, projectId: pid })
            .then(function () {
